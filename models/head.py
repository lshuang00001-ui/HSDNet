...
__all__ = ['Detect_DyHead', 'Detect_DyHeadWithDCNV3', 'Detect_DyHeadWithDCNV4', 'Detect_AFPN_P345', 'Detect_AFPN_P345_Custom', 'Detect_AFPN_P2345', 'Detect_AFPN_P2345_Custom', 'Detect_Efficient', 'DetectAux',
           'Segment_Efficient', 'Detect_SEAM', 'Detect_MultiSEAM', 'Detect_DyHead_Prune', 'Detect_SEPS', 'Segment_LSCD', 'Pose_LSCD', 'OBB_LSCD', 'Detect_TADDH', 'Segment_TADDH', 'Pose_TADDH', 'OBB_TADDH',
...
# Assumed existing helpers: Conv_GN, Scale, make_anchors, dist2bbox, DFL
...
#
#         # per-layer backbone conv->GN->ReLU
#         self.conv = nn.ModuleList([nn.Sequential(Conv_GN(x, hidc, 3), nn.ReLU(inplace=True)) for x in ch])
#         # shared conv + SE
...
#         self.dfl = DFL(self.reg_max) if self.reg_max > 1 else nn.Identity()
#
#     def _is_tf_format(self):
#         return self.format in ("saved_model", "pb", "tflite", "edgetpu", "tfjs")
#
#     def forward(self, x):
#
#         for i in range(self.nl):
#             y = self.conv[i](x[i])
#             y = self.share_conv(y)
#             y = torch.cat((self.scale[i](self.cv2(y)), self.cv3(y)), 1)
#             x[i] = y
#         # print(f"[LOG] 鉁?Using modified Detect_SEPS with SEBlock | nc={self.nc}, hidc={self.share_conv[0].conv.in_channels}")  鍙鍖栦慨鏀?#         if self.training:
#             return x
#
#         # Inference path
#         b, _, _, _ = x[0].shape
...
#
#         # split predictions
#         box_feat, cls_feat = x_cat.split((self.reg_max * 4, self.nc), dim=1) if not (self.export and self._is_tf_format()) \
#             else (x_cat[:, : self.reg_max * 4], x_cat[:, self.reg_max * 4 :])
#
#         dbox = self.decode_bboxes(box_feat)
#
#         if self.export and self.format in ("tflite", "edgetpu"):
#             img_h, img_w = self.shape[2], self.shape[3]
#             img_size = torch.tensor([img_w, img_h, img_w, img_h], device=box_feat.device).view(1, 4, 1)
#             norm = self.strides / (self.stride[0] * img_size)
#             dbox = dist2bbox(
#                 self.dfl(box_feat) * norm,
#                 self.anchors.unsqueeze(0) * norm[:, :2],
#                 xywh=True,
#                 dim=1,
#             )
#
#         y = torch.cat((dbox, cls_feat.sigmoid()), dim=1)
#         return y if self.export else (y, x)
#
#     def bias_init(self, cf=None):
#         # initialize box and class biases
#         self.cv2.bias.data.fill_(1.0)
#         if cf is None:
#             bias_val = math.log(5 / self.nc / (640 / 16) ** 2)
#             self.cv3[-1].bias.data[:] = bias_val
#         else:
#             # cf: tensor of class frequencies
#             freq = cf.float() / cf.sum()
#             bias = torch.log(freq.clamp(min=1e-4))
#             self.cv3[-1].bias.data[: self.nc] = bias
#
...
class Segment_LSCD(Detect_SEPS):
    """YOLOv8 Segment head for segmentation models."""
    ...
    def __init__(self, nc=80, nm=32, npr=256, hidc=256, ch=()):
        ...
        self.npr = npr  # number of protos
        self.proto = Proto(ch[0], self.npr, self.nm)  # protos
        self.detect = Detect_SEPS.forward
...
class LQE(nn.Module):
    """
    浣嶇疆璐ㄩ噺浼拌鍣?(Location Quality Estimator, LQE)
    鐢ㄤ簬璇勪及鍜岃皟鏁磋竟鐣屾棰勬祴鐨勮川閲忓垎鏁帮紝缁撳悎鍒嗗竷缁熻淇℃伅鎻愬崌绮惧害
    """
    def __init__(self, k, hidden_dim, num_layers, reg_max):
        """
        鍒濆鍖?LQE 妯″潡
        鍙傛暟:
            k: 鍓?k 涓渶楂樻鐜囧€肩殑鏁伴噺锛岀敤浜庣粺璁″垎鏋?            hidden_dim: MLP闅愯棌灞傜淮搴?            num_layers: MLP灞傛暟
            reg_max: 鍥炲綊鐨勬渶澶у€硷紙杈圭晫妗嗗垎甯冪殑鏈€澶ц寖鍥达級
        """
        super(LQE, self).__init__()
        self.k = k
        self.reg_max = reg_max
        # 瀹氫箟涓€涓灞傛劅鐭ユ満锛圡LP锛夛紝杈撳叆缁村害涓?4*(k+1)锛岃緭鍑轰负 1
        self.reg_conf = MLP(4 * (k + 1), hidden_dim, 1, num_layers)
        # 鍒濆鍖栨渶鍚庝竴灞傜殑鍋忕疆鍜屾潈閲嶄负 0
    ...
    def forward(self, scores, pred_corners):
        ...
        鍓嶅悜浼犳挱
        鍙傛暟:
            scores: 鍒濆鍒嗙被寰楀垎 [B, num_classes, h, w]
            pred_corners: 棰勬祴鐨勮竟鐣屾瑙掔偣鍒嗗竷 [B, 4*(reg_max), h, w]
        杩斿洖:
            璋冩暣鍚庣殑璐ㄩ噺鍒嗘暟
        """
        # 璁＄畻 softmax 姒傜巼
        ...
        # 鎻愬彇鍓?k 涓渶楂樻鐜囧€煎強鍏剁储寮?        prob_topk, _ = prob.topk(self.k, dim=1)
        # 灏?top-k 姒傜巼鍙婂叾鍧囧€兼嫾鎺ワ紝浣滀负缁熻鐗瑰緛
        stat = torch.cat([prob_topk, prob_topk.mean(dim=1, keepdim=True)], dim=1)
        # 閫氳繃 MLP 璁＄畻璐ㄩ噺鍒嗘暟璋冩暣鍊?        quality_score = self.reg_conf(stat.reshape(B, -1, H, W))
        # 灏嗗垵濮嬪緱鍒嗕笌璐ㄩ噺璋冩暣鍊肩浉鍔?        return scores + quality_score
