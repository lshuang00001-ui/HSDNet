...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C3k2_Faster', 'C3k2_ODConv', 'Partial_conv3', 'C3k2_Faster_EMA', 'C3k2_DBB',
           ...
           'C3k2_ContextGuided', 'C3k2_MSBlock', 'ContextGuidedBlock_Down', 'C3k2_DLKA', 'CSPStage', 'SE_SPDConv',
           ...
           'C3k2_SMPCGLU', 'C3k2_Heat', 'SBA', 'WaveletPool', 'WaveletUnPool', 'CSP_PTB', 'GLSA', 'SplitOmniFusion', 'WTConv2d', 'C3k2_WTConv',
           ...
           'C3k2_MambaOut_SFSC', 'C3k2_PSFSConv', 'C3k2_FasterSFSC', 'FCM', 'FCM_1', 'FCM_2', 'FCM_3', 'Pzconv', 'C3k2_GroupMamba', 'C3k2_GroupMambaBlock', 'C3k2_MambaVision', 'C3k2_wConv', 'wConv2d', 'PST', 'C3k2_FourierConv', 'FourierConv', 'C3k2_GLVSS',
           ]
...
# Assuming Conv and SEBlock are defined elsewhere in your codebase
# You can replace Conv with Conv_GN or similar if needed

# class SEBlock(nn.Module):
#     def __init__(self, channels: int):
#         super().__init__()
#         internal_neurons = max(8, channels // 16)
...
#
#     def forward(self, x):
#         print(f"[LOG] 鉁?SEBlock activated | input shape: {x.shape}")
...
# class SE_SPDConv(nn.Module):
#     def __init__(self, inc, ouc, dimension=1):
#         super().__init__()
#         print(f"[SE_SPDConv] initialized with SEBlock 鉁?| in_channels={inc}, out_channels={ouc}, dimension={dimension}")
...
#
#     def forward(self, x):
#         print(f"[LOG] 鉁?SE_SPDConv activated | input shape: {x.shape}")
#         x = self.unshuffle(x)
#         return self.conv(x)
...
class AKConv(nn.Module):
    ...
    def _reshape_x_offset(x_offset, num_param):
        ...
        # using 1 脳 1 Conv
        # x_offset = x_offset.permute(0,1,4,2,3), then, x_offset.view(b,c脳num_param,h,w)  finally, Conv2d(c脳num_param,c_out, kernel_size =1,stride=1,bias= False)
        # using the column conv as follow锛?then, Conv2d(inc, outc, kernel_size=(num_param, 1), stride=(num_param, 1), bias=bias)
...
class DySample(nn.Module):
    """
    Dynamic Sampling Upsampling Module (鏀硅繘鐗?
    --------------------------------------------------
    - 鏀寔 'lp' 涓?'pl' 涓ょ妯″紡锛?    - 鏀硅繘鍒濆鍖栫瓥鐣ワ紝鏇寸ǔ瀹氱殑璁粌锛?    - 寮曞叆鍙涔犵殑 offset_scale锛?    - 缂撳瓨 base grid 鎻愬崌鎺ㄧ悊閫熷害锛?    - 浠ｇ爜鏁村悎锛岄€昏緫鏇寸畝娲侊紱
    - 鏀寔 groups 鍒嗙粍鍗风Н浠ユ彁鍗囨晥鐜囥€?    """
    ...
    def __init__(self, in_channels, scale=2, style='lp', groups=4, dyscope=False):
        ...
        # 鍔ㄦ€佸亸绉婚娴嬪嵎绉?        self.offset = nn.Conv2d(in_channels, out_channels, kernel_size=1, groups=groups)
        nn.init.kaiming_uniform_(self.offset.weight, a=1)
        nn.init.constant_(self.offset.bias, 0)

        # 鍙涔犲亸绉荤缉鏀惧弬鏁?        self.offset_scale = nn.Parameter(torch.tensor(0.25))

        # Scope 妯″潡锛堝彲閫夛紝杩涗竴姝ユ帶鍒?offset 鑼冨洿锛?        if dyscope:
            self.scope = nn.Conv2d(in_channels, out_channels, kernel_size=1, groups=groups)
            nn.init.constant_(self.scope.weight, 0)
            nn.init.constant_(self.scope.bias, 0)

        # 浣嶇疆鍒濆鍖?        self.register_buffer('init_pos', self._init_pos())
        self.register_buffer('base_coords', None)

    # ---------------------------------------------------------
    # 鍒濆鍖栧熀纭€鍧愭爣
    # ---------------------------------------------------------
    def _init_pos(self):
        h = torch.linspace(-(self.scale - 1) / (2 * self.scale),
                           (self.scale - 1) / (2 * self.scale),
                           self.scale)
        gx, gy = torch.meshgrid(h, h, indexing='ij')
        base = torch.stack([gx, gy], dim=0).flatten(1).unsqueeze(0).repeat(1, self.groups, 1, 1)
    ...
    def _get_coords(self, H, W, device, dtype):
        if self.base_coords is None or self.base_coords.shape[-2:] != (H, W):
            yy, xx = torch.meshgrid(torch.arange(H), torch.arange(W), indexing='ij')
            coords = torch.stack([xx + 0.5, yy + 0.5], dim=0).unsqueeze(0).unsqueeze(0)
            self.base_coords = coords
        return self.base_coords.to(device=device, dtype=dtype)
    ...
    def forward(self, x):
        ...
        # 鍙€?scope
        scope = self.scope(x).sigmoid() if hasattr(self, 'scope') else 1.0
        offset = self.offset(x) * scope * self.offset_scale + self.init_pos
...
class CFC_CRB(nn.Module):
    def __init__(self, in_channels=512, grids=(6, 3, 2, 1)): # 鍏坈e鍚巉fm
    ...
    def forward(self, x):
        ...
        x = self.reduce_channel(x) # 闄嶇淮- 128
...
class SFC_G2(nn.Module):
    ...
    def forward(self, x):
        ...
        # x_32
        sp = self.conv_32(sp)  # 璇箟鐗瑰緛  1 / 8  256
        ...
        cp = F.grid_sample(cp, grid_l , align_corners=True)  ## 鑰冭檻鏄惁鎸囧畾align_corners
        sp = F.grid_sample(sp, grid_h , align_corners=True)  ## 鑰冭檻鏄惁鎸囧畾align_corners
...
class SDFM(nn.Module):
    ...
    def forward(self, data):
        ...
        recal_input = recal_w * input ## 鍏堝鐗瑰緛杩涜涓€姝ヨ嚜鏍℃
        recal_input = recal_input + input
        x1, x2 = torch.split(recal_input, c, dim =1)
        agg_input = self.channel_agg(recal_input) ## 杩涜鐗瑰緛鍘嬬缉 鍥犱负鍙绠椾竴涓壒寰佺殑鏉冮噸
        local_w = self.local_att(agg_input)  ## 灞€閮ㄦ敞鎰忓姏 鍗硈patial attention
        global_w = self.global_att(agg_input) ## 鍏ㄥ眬娉ㄦ剰鍔?鍗砪hannel attention
        w = self.sigmoid(local_w * global_w) ## 璁＄畻鐗瑰緛x1鐨勬潈閲?
        xo = w * x1 + (1 - w) * x2 ## fusion results ## 鐗瑰緛鑱氬悎
        return xo
...
class OmniKernelLite(nn.Module):
    def __init__(self, dim, reduction=4):
        ...
        # Depthwise directional convolutions
        self.dw_13 = nn.Conv2d(dim, dim, kernel_size=(1, 7), padding=(0, 3), groups=dim)
        self.dw_31 = nn.Conv2d(dim, dim, kernel_size=(7, 1), padding=(3, 0), groups=dim)

        # Lightweight SE-style attention
    ...
    def forward(self, x):
        ...
        # Output
        x = self.out_conv(x)
        x = self.act(x + res)  # Add residual for stability
...
# class SplitOmniFusion(nn.Module):
#     def __init__(self, dim: int, e: float = 0.25):
#         super().__init__()
#         self.e = e
#         self.cv1 = Conv(dim, dim, 1)
#         self.cv2 = Conv(dim, dim, 1)
#
#         self.ok_channels = int(dim * self.e)
#         self.identity_channels = dim - self.ok_channels
#
#         self.m = OmniKernelLite(self.ok_channels)
#
#     def forward(self, x: torch.Tensor) -> torch.Tensor:
#         x_proj = self.cv1(x)
#         ok_branch, identity = torch.split(x_proj, [self.ok_channels, self.identity_channels], dim=1)
#         fused = torch.cat((self.m(ok_branch), identity), dim=1)
...
class AdditiveTokenMixer(nn.Module):
    """
    鏀瑰彉浜唒roj鍑芥暟鐨勮緭鍏ワ紝涓嶅q+k鍗风Н锛岃€屾槸瀵硅瀺鍚堜箣鍚庣殑缁撴灉proj
    """
...
# def safe_channel_shuffle(x: torch.Tensor, groups: int) -> torch.Tensor:
#     b, c, h, w = x.size()
#     if groups is None or groups <= 1 or (c % groups) != 0:
#         return x
...
# class EUCB(nn.Module):
#     """
#     Upsample -> Depthwise Conv -> Channel Shuffle -> Pointwise Conv
#     """
#     def __init__(self, in_channels, kernel_size=3, stride=1, shuffle_groups=2, upsample_mode="nearest"):
#         super(EUCB, self).__init__()
#         assert kernel_size in (1, 3, 5, 7), "kernel_size should be odd and small for DW conv"
...
#         )
#         self.up_dwc = Conv(in_channels, in_channels, kernel_size, g=in_channels, s=stride)
#         # bias=False assuming Conv includes BN
#         self.pwc = nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, padding=0, bias=False)

#     def forward(self, x):
#         x = self.upsample(x)
#         x = self.up_dwc(x)
#         x = safe_channel_shuffle(x, self.shuffle_groups)
#         x = self.pwc(x)
#         return x
...
#     If dw_parallel=True: all branches take the same input.
#     If dw_parallel=False: branches are applied serially with residual accumulation.
#     """
#     def __init__(self, in_channels, kernel_sizes=(1, 3, 5), stride=1, dw_parallel=True):
#         super(MSDC, self).__init__()
#         self.in_channels = in_channels
...
#         self.dwconvs = nn.ModuleList([
#             Conv(in_channels, in_channels, k, s=stride, g=in_channels)
#             for k in self.kernel_sizes
#         ])

#     def forward(self, x):
#         if self.dw_parallel:
#             outs = [m(x) for m in self.dwconvs]
#             return outs
#         else:
#             outs, h = [], x
#             for m in self.dwconvs:
#                 y = m(h)
#                 h = h + y
#                 outs.append(h)
#             return outs
...
#     1x1 expand -> multi-scale DW -> (sum or cat) -> shuffle -> 1x1 project -> residual (if stride=1)
#     """
#     def __init__(self,
#                  in_channels,
#                  out_channels,
...
#         ex_channels = int(round(in_channels * self.expansion_factor))
#         self.pconv1 = Conv(in_channels, ex_channels, 1)

#         self.msdc = MSDC(ex_channels, self.kernel_sizes, stride=self.stride, dw_parallel=self.dw_parallel)

#         self.combined_channels = ex_channels if add else ex_channels * len(self.kernel_sizes)
#         self.pconv2 = Conv(self.combined_channels, out_channels, 1, act=False)
...
#         if self.add:
#             merged = torch.stack(outs, dim=0).sum(dim=0)
#         else:
#             merged = torch.cat(outs, dim=1)

#         merged = safe_channel_shuffle(merged, self.shuffle_groups)
#         out = self.pconv2(merged)
...
# class CSP_MSCB(C2f):
#     """
#     C2f wrapper to stack MSCB blocks.
#     """
#     def __init__(self, c1, c2, n=1, kernel_sizes=(1, 3, 5), shortcut=False, g=1, e=0.5,
#                  expansion_factor=2.0, dw_parallel=True, add=True, shuffle_groups=2):
#         super().__init__(c1, c2, n, shortcut, g, e)
...
class SMA(nn.Module):
    ...
    def forward(self, value, key, query):
        ...
        # 灏嗚緭鍑鸿浆鎹负閫傚悎AttentionBlock鐨勮緭鍏ユ牸寮?        batch_size, seq_len, feature_size = MSA.shape
        MSA = MSA.permute(0, 2, 1).view(batch_size, feature_size, int(seq_len**0.5), int(seq_len**0.5))
        # 閫氳繃CombinedModulator杩涜multi-attn fusion
        synergistic_attn = self.combined_modulator.forward(MSA)


        # 灏嗚緭鍑鸿浆鎹㈠洖 (batch_size, seq_len, feature_size) 鏍煎紡
...
# 1.浣跨敤 nn.AvgPool2d 瀵硅緭鍏ョ壒寰佸浘杩涜骞虫粦鎿嶄綔锛屾彁鍙栧叾浣庨淇℃伅銆?# 2.灏嗗師濮嬭緭鍏ョ壒寰佸浘涓庡钩婊戝悗鐨勭壒寰佸浘杩涜鐩稿噺锛屽緱鍒板寮虹殑杈圭紭淇℃伅锛堥珮棰戜俊鎭級銆?# 3.鐢ㄥ嵎绉搷浣滆繘涓€姝ュ鐞嗗寮虹殑杈圭紭淇℃伅銆?# 4.灏嗗鐞嗗悗鐨勮竟缂樹俊鎭笌鍘熷杈撳叆鐗瑰緛鍥剧浉鍔狅紝浠ュ舰鎴愬寮哄悗鐨勮緭鍑恒€?class EdgeEnhancer(nn.Module):
...
class ScharrConv(nn.Module):
    def __init__(self, channel):
        ...
        # 瀹氫箟Scharr绠楀瓙鐨勬按骞冲拰鍨傜洿鍗风Н鏍?        scharr_kernel_x = np.array([[3,  0, -3],
                                    [10, 0, -10],
        ...
        # 灏哠charr鏍歌浆鎹负PyTorch寮犻噺骞舵墿灞曚负閫氶亾鏁?        scharr_kernel_x = torch.tensor(scharr_kernel_x, dtype=torch.float32).unsqueeze(0).unsqueeze(0)  # (1, 1, 3, 3)
        ...
        # 瀹氫箟鍗风Н灞傦紝浣嗕笉瀛︿範鍗风Н鏍革紝鐩存帴浣跨敤Scharr鏍?        self.scharr_kernel_x_conv = nn.Conv2d(channel, channel, kernel_size=3, padding=1, groups=channel, bias=False)
        self.scharr_kernel_y_conv = nn.Conv2d(channel, channel, kernel_size=3, padding=1, groups=channel, bias=False)
        
        # 灏嗗嵎绉牳鐨勬潈閲嶈缃负Scharr绠楀瓙鐨勬牳
    ...
    def forward(self, x):
        # 瀵硅緭鍏ョ殑鐗瑰緛鍥捐繘琛孲charr鍗风Н锛堟按骞冲拰鍨傜洿鏂瑰悜锛?        grad_x = self.scharr_kernel_x_conv(x)
        grad_y = self.scharr_kernel_y_conv(x)
        
        # 璁＄畻姊害骞呭€?        edge_magnitude = grad_x * 0.5 + grad_y * 0.5
...
class FreqSpatial(nn.Module):
    ...
    def forward(self, x):
        ...
        # 3. 杩樺師鍥炴椂鍩?        fft_feat = rearrange(fft_feat, 'b (c d) h w -> b c h w d', d=2).contiguous()
        ...
        # 鍚堝苟鏃跺煙鍜岄鍩熺壒寰?        out = spatial_feat + fft_feat
...
class HaarWaveletConv(nn.Module):
    ...
    def forward(self, x):
        ...
        # a (approximation): 浣庨淇℃伅锛屽浘鍍忕殑骞虫粦閮ㄥ垎锛屼唬琛ㄤ簡鍥惧儚鐨勬暣浣撶粨鏋勩€?        # h (horizontal): 姘村钩鏂瑰悜鐨勯珮棰戜俊鎭紝鎹曟崏姘村钩鏂瑰悜涓婄殑杈圭紭鎴栧彉鍖栥€?        # v (vertical): 鍨傜洿鏂瑰悜鐨勯珮棰戜俊鎭紝鎹曟崏鍨傜洿鏂瑰悜涓婄殑杈圭紭鎴栧彉鍖栥€?        # d (diagonal): 瀵硅绾挎柟鍚戠殑楂橀淇℃伅锛屾崟鎹夊瑙掔嚎鏂瑰悜涓婄殑杈圭紭鎴栫汗鐞嗐€?        a, h, v, d = out.chunk(4, 1)
        
        # 浣庨锛岄珮棰?        return a, h + v + d
...
######################################## ICLR2025 Kolmogorov鈥揂rnold Transformer start ########################################
...
######################################## ICLR2025 Kolmogorov鈥揂rnold Transformer end ########################################
...
# Shift_channel_mix 妯″潡锛?# 鏈爺绌舵彁鍑轰簡涓€绉嶈交閲忕骇鐗瑰緛娣峰悎妯″潡 Shift_channel_mix锛屾棬鍦ㄩ€氳繃閫氶亾鍒嗗壊涓庣┖闂村亸绉绘搷浣滃寮虹壒寰佽〃杈捐兘鍔涖€?# 鍏蜂綋鑰岃█锛岃妯″潡棣栧厛娌跨潃閫氶亾缁村害锛坉im=1锛夊杈撳叆鐗瑰緛鍥捐繘琛屽洓绛夊垎鍒嗗潡锛堝嵆 x_1, x_2, x_3, x_4锛夛紝闅忓悗鍒嗗埆鍦ㄦ按骞虫柟鍚戯紙瀹藉害缁村害锛夊拰鍨傜洿鏂瑰悜锛堥珮搴︾淮搴︼級涓婃柦鍔犳璐熸柟鍚戠殑寰幆绉讳綅锛坈ircular shift锛夈€?# 鍏朵腑锛寈_1 鍜?x_2 鍒嗗埆鍦ㄩ珮搴︽柟鍚戣繘琛屾鍚戝拰璐熷悜鍋忕Щ锛岃€?x_3 鍜?x_4 鍒欏湪瀹藉害鏂瑰悜杩涜姝ｅ悜鍜岃礋鍚戝亸绉汇€?# 鏈€缁堬紝鍋忕Щ鍚庣殑鐗瑰緛鍧楅€氳繃閫氶亾鎷兼帴锛坈hannel concatenation锛夐噸鏂扮粍鍚堬紝浠ュ疄鐜拌法閫氶亾鐨勪俊鎭氦浜掍笌灞€閮ㄧ壒寰佸寮恒€?
# 璇ヨ璁＄殑鏍稿績鎬濇兂鏄埄鐢ㄩ€氶亾鍐呬俊鎭噸鍒嗗竷鐨勬柟寮忥紝寮曞涓嶅悓閫氶亾鐗瑰緛鎰熷彈涓嶅悓鐨勭┖闂翠綅缃俊鎭紝浠庤€屾彁鍗囩綉缁滅殑鐗瑰緛琛ㄨ揪鑳藉姏銆?# 姝ゅ锛岀敱浜庤鎿嶄綔浠呮秹鍙婂熀鏈殑閫氶亾鍒囧垎涓庡惊鐜Щ浣嶏紝璁＄畻澶嶆潅搴︽瀬浣庯紝涓嶅紩鍏ラ澶栫殑鍙傛暟鎴栨樉钁楃殑璁＄畻寮€閿€銆?# 鍥犳锛孲hift_channel_mix 閫傜敤浜庡璁＄畻璧勬簮鍙楅檺鐨勪换鍔★紝濡傚祵鍏ュ紡瑙嗚绯荤粺鎴栧疄鏃剁洰鏍囨娴嬬瓑鍦烘櫙銆?class Shift_channel_mix(nn.Module):
...
class Scharr(nn.Module):
    def __init__(self, channel):
        ...
        # 瀹氫箟Scharr婊ゆ尝鍣?        scharr_x = torch.tensor([[-3., 0., 3.], [-10., 0., 10.], [-3., 0., 3.]], dtype=torch.float32).unsqueeze(0).unsqueeze(0)
        ...
        # 灏哠obel婊ゆ尝鍣ㄥ垎閰嶇粰鍗风Н灞?        self.conv_x.weight.data = scharr_x.repeat(channel, 1, 1, 1)
    ...
    def forward(self, x):
        ...
        # 璁＄畻杈圭紭鍜岄珮鏂垎甯冨己搴︼紙鍙互閫夋嫨涓嶅悓鐨勬柟寮忚繘琛岃瀺鍚堬紝杩欓噷浣跨敤骞虫柟鍜屽紑鏍瑰彿锛?        scharr_edge = torch.sqrt(edges_x ** 2 + edges_y ** 2)
...
class LoGFilter(nn.Module):
    def __init__(self, in_c, out_c, kernel_size, sigma):
        ...
        """鍒涘缓楂樻柉-鎷夋櫘鎷夋柉鏍?""
        # 鍒濆鍖栦簩缁村潗鏍?        ax = torch.arange(-(kernel_size // 2), (kernel_size // 2) + 1, dtype=torch.float32)
        xx, yy = torch.meshgrid(ax, ax)
        # 璁＄畻楂樻柉-鎷夋櫘鎷夋柉鏍?        kernel = (xx**2 + yy**2 - 2 * sigma**2) / (2 * math.pi * sigma**4) * torch.exp(-(xx**2 + yy**2) / (2 * sigma**2))
        # 褰掍竴鍖?        kernel = kernel - kernel.mean()
        kernel = kernel / kernel.sum()
        log_kernel = kernel.unsqueeze(0).unsqueeze(0) # 娣诲姞 batch 鍜?channel 缁村害
...
class LoGStem(nn.Module):
    ...
    def __init__(self, in_chans, stem_dim):
        ...
        # 瀹氫箟LoG婊ゆ尝鍣?        self.LoG = LoGFilter(in_chans, out_c14, 7, 1.0)
        # gaussian
